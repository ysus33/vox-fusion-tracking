...
class VoxMapping:
    ...
    def start(self):
        mapping_process = mp.Process(
            target=self.mapper.spin_for_pure_mapping, args=(self.share_data, self.kf_buffer))
        mapping_process.start()
        print("initializing the first frame ...")
        tracking_process = mp.Process(
            target=self.tracker.spin_for_pure_mapping, args=(self.share_data, self.kf_buffer))
